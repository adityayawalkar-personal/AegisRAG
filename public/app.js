...
function renderGeneratorBadge(generatedBy) {
  if (generatedBy === 'local_gemma_server') {
    return `<span class="gen-badge gen-badge-local" title="Generated by Local llama.cpp Gemma 4 E2B">🟢 Local Gemma 4 E2B</span>`;
  }
  if (generatedBy === 'huggingface_inference') {
    return `<span class="gen-badge gen-badge-hf" title="Generated by Hugging Face Inference API">🟡 HF Inference (cloud fallback)</span>`;
  }
  return `<span class="gen-badge gen-badge-fallback" title="Generated by Deterministic Rule-Based Fallback">🔴 Deterministic fallback — Gemma unreachable</span>`;
}
  ...
  container.innerHTML = heals
    .map((h) => {
      ...
      <div class="heal-card">
        ...
        <div style="font-size:13px; color:var(--text-secondary); display:flex; align-items:center; flex-wrap:wrap; gap:6px;">
          <strong>Gemma AI Repair Diagnosis:</strong>
          ${renderGeneratorBadge(h.generated_by)}
        </div>
        <p style="font-size:13px; color:var(--text-primary); font-style:italic; margin-top:2px;">
          "${escapeHtml(h.heal_description)}"
        </p>

        <div style="font-size:12px; color:var(--text-muted); margin-top:6px;">Preview Extractions (Synthesized CSS Selectors):</div>
...
async function fetchIncidentReplay() {
  try {
    ...
    container.innerHTML = data.timeline
      .map((event) => {
        let badgeHtml = '';
        if (event.type === 'HEAL_DIAGNOSIS' && event.details?.generatedBy) {
          badgeHtml = renderGeneratorBadge(event.details.generatedBy);
        }

        let descText = event.details?.description || event.description || '';
        ...
        return `
          <div class="timeline-item">
            <div class="timeline-content">
              ...
              <div class="timeline-title" style="display:flex; align-items:center; flex-wrap:wrap; gap:6px;">
