...
3. **Safe Bright Data Self-Healing**: Feeds the diagnosis into `bdata scraper heal` via safe `execFile` argument arrays, previews the regenerated extraction, and halts at a human-in-the-loop approval gate.
4. **Self-Cleaning Knowledge Store**: Bumping schema versions atomically purges superseded extractions from both dense vector and Okapi BM25 indices, eliminating index drift.
5. **Verifiable Hybrid Retrieval**: Combines semantic embeddings and BM25 via Reciprocal Rank Fusion (RRF, $k=60$), expands chunks to parent sections, and deterministically enforces inline source citations with verified timestamps.
...
│   • The Self-Healing Loop (Gemma Diagnosis, CLI Wrappers, Approval Gate)    │
...
│   • Bright Data Scraper Studio CLI (@brightdata/cli@1.2.0)                   │
...
- `bdata scraper run <collector_id> --url <target_url>`: Executes live web extractions and returns structured JSON payloads.
- `bdata scraper heal <collector_id> "<gemma_description>" --url <target_url>`: Feeds Gemma's plain-language diagnosis to Scraper Studio's generative AI to synthesize updated CSS selectors and browser automation logic.
...
| **Validation & Healing** | Interactive code repair candidates (`preview_result` envelope). | **The Sentinel** validation engine, 5-run median baseline comparison, 20% noise threshold, Gemma diagnosis prompt client, 3-strike circuit breaker, and state machine. |
...
sequenceDiagram
    ...
    Runner->>BD: bdata scraper run (safe execFile)
...
5. Runs the full test suite (47 tests).
...
# Run Full Vitest Test Suite (47 tests)
npm test
...
| `src/healing/gemma-client.ts` | **Heavily Edited by Us** | Local Gemma inference client enforcing single-sentence <900 character repair prompts. |
| `src/healing/heal-loop.ts` | **Hand-Written by Us** | Safe `execFile` argument-array CLI wrappers with manual `approveHeal`/`rejectHeal` gates. |
...
| `src/server/app.ts` & `auth.ts` | **Hand-Written by Us** | Native Node.js REST API with token authentication (`x-api-key`). |
| `public/*` | **Heavily Edited by Us** | Modern dark-theme dashboard (Chat, Health Console, Incident Replay) with XSS sanitization. |
| `tests/*.test.ts` (13 suites) | **Heavily Edited by Us** | 47 automated unit tests covering all edge cases, synthetic failures, and RAG pipelines. |
...
1. **Safe CLI Invocations**: CLI calls with data from scraped content or LLM output use `execFile` argument arrays — never string concatenation into a shell.
2. **Strict Secrets Hygiene**: Zero credentials in git; `.env` is strictly git-ignored; logs redact API keys.
...
5. **Untrusted Data Isolation**: Scraped web text is wrapped in `<RETRIEVED_CONTEXT>` tags and treated strictly as passive reference data.
